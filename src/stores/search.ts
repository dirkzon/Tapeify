...
export const useSearchStore = defineStore(STORE_NAME, {
  ...
  actions: {
    async SearchPlaylistsAndAlbums(query: string): Promise<void> {
        ...
        const nextPageAvailable = (result['albums']['next'] == null && result['playlists']['next'] == null)
        const previousPageAvailable =(result['albums']['previous'] == null && result['playlists']['previous'] == null)
